...
def read_wav_data(filename):
    ...
    print('channel is {}'.format(num_channel))
    print('frame is {}'.format(num_frame))
    print('sample_width  is {}'.format(num_sample_width))
    print('framerate is {}'.format(framerate))
    str_data = wav.readframes(num_frame) # 读取全部的帧
    wav.close() # 关闭流
    wave_data = np.fromstring(str_data, dtype = np.short) # 将声音文件数据转换为数组矩阵形式
    # wave_data=wave_data[50000:210000]
...
if(__name__=='__main__'):
	...
	wave_data, fs = read_wav_data("/home/wanggf/Github/ASRT_SpeechRecognition/dataset/data_thchs30/data/A2_3.wav")
